...
fn test_security_level_consistency() {
    ...
    for level in [SecurityLevel::Low, SecurityLevel::Medium, SecurityLevel::High, SecurityLevel::Critical] {
        for category in [
            ...
            assert!(!disclaimer.is_empty(), "Disclaimer should not be empty for level {:?} and category {:?}", level, category);
            ...
            match category {
                ...
                DisclaimerCategory::Upgrade => assert!(disclaimer.contains("UPGRADE WARNING") || disclaimer.contains("UPGRADE INFO")),
...
fn test_multi_contract_security_levels() {
    ...
    let low_contract_disclaimer = SecurityDisclaimer::get_disclaimer(env.clone(), SecurityLevel::Low, DisclaimerCategory::Audit);
    let critical_contract_disclaimer = SecurityDisclaimer::get_disclaimer(env.clone(), SecurityLevel::Critical, DisclaimerCategory::Audit);
...
fn test_disclaimer_content_validation() {
    ...
    let critical_disclaimer = SecurityDisclaimer::get_disclaimer(env.clone(), SecurityLevel::Critical, DisclaimerCategory::Audit);
    let high_disclaimer = SecurityDisclaimer::get_disclaimer(env.clone(), SecurityLevel::High, DisclaimerCategory::Audit);
    let medium_disclaimer = SecurityDisclaimer::get_disclaimer(env.clone(), SecurityLevel::Medium, DisclaimerCategory::Audit);
    let low_disclaimer = SecurityDisclaimer::get_disclaimer(env.clone(), SecurityLevel::Low, DisclaimerCategory::Audit);
